...
class Tml1Model extends CI_Model {
  ...
  function get_where($con = array(), $str_condition = null) {
    if ($con) {
      $this->template_db->where($con);
    }
    if ($str_condition != null) {
      $this->template_db->where($str_condition);
    }
	$this->template_db->order_by('TML1_Org_ID, Sequence', 'ASC');
    $this->template_db->from($this->table);
    return $this->template_db->get();
  }
